            ...
            steps {
                ...
                withCredentials([usernamePassword(
                    ...
                    sh '''
                        ...
                        CONFIG_DIR=$(mktemp -d)
                        printf '{"auths":{"https://index.docker.io/v1/":{"auth":"%s"}}}' "$AUTH" > $CONFIG_DIR/config.json
                        docker --config $CONFIG_DIR push $DOCKER_IMAGE:$DOCKER_TAG
                        docker --config $CONFIG_DIR push $DOCKER_IMAGE:latest
                        rm -rf $CONFIG_DIR
                    '''
